...
Исторически первые коммиты создавались в отдельной ветке `work`, поэтому при клонировании репозитория могла возникать путаница. Теперь вся разработка ведётся напрямую в `main`; дополнительных веток в проекте нет. Если локально всё ещё осталась ветка `work`, её можно безопасно удалить после переключения на `main`:
...
После этого рекомендуется заново подтянуть изменения из удалённого `main` и продолжать работу только в этой ветке.
